...
import ru.skillbranch.devintensive.extensions.toUserItem
...
class GroupViewModel : ViewModel() {
    ...
    fun handleSelectedItem(userId: String) {
        userItems.value = userItems.value?.map { if (it.id == userId) it.copy(isSelected = !it.isSelected) else it } ?: listOf()
    }

    fun handleRemoveChip(userId: String) {
        userItems.value = userItems.value?.map { if (it.id == userId) it.copy(isSelected = false) else it } ?: listOf()
    }
