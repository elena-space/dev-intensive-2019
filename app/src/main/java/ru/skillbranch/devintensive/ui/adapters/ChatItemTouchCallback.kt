...
class ChatItemTouchCallback(private val adapter: ChatAdapter,
    ...
    override fun getMovementFlags(recyclerView: RecyclerView, viewHolder: RecyclerView.ViewHolder): Int = when (viewHolder) {
    ...
    override fun onSwiped(viewHolder: RecyclerView.ViewHolder, direction: Int) = swipeListener.invoke(adapter.items[viewHolder.adapterPosition])

    override fun onSelectedChanged(viewHolder: RecyclerView.ViewHolder?, actionState: Int) {
        if (actionState != ItemTouchHelper.ACTION_STATE_IDLE && viewHolder is ItemTouchViewHolder) viewHolder.onItemSelected()
    ...
    private fun drawBackground(canvas: Canvas, itemView: View, dX: Float) {
        ...
        with(bgPaint) { color = itemView.resources.getColor(R.color.color_primary_dark, itemView.context.theme) }
    ...
    private fun drawIcon(canvas: Canvas, itemView: View, dX: Float) {
        ...
        val space = itemView.resources.getDimensionPixelSize(R.dimen.spacing_16)
